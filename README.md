...
- **🖥️ 配置文件驱动共享**：无参数运行自动读取（不存在则创建）用户配置目录下的默认配置文件 `filespace/config.yaml`，按其 `shared_folders` 共享；也可用 `-d/--dir` 临时指定共享文件夹；本机管理页添加/移除的共享会同步写回配置文件，命令行加 `--save` 也可把本次参数设置（目录/密码/端口）追加保存到配置文件
...
**命令行 `--save`**：把本次命令行参数设置**追加**保存到当前配置文件后再继续运行，下次无参数启动即按新配置生效——`-d/--dir` 目录追加进 `shared_folders`（去重，保留原有条目与各文件夹显式密码）、`-P/--passwd` 覆盖顶层默认密码（`-P ''` 清除）、`-p/--port` 覆盖监听端口。例：`filespace -d ~/docs -P secret --save` 共享 `~/docs`、设默认密码并一并保存；`filespace -p 9000 --save` 只把端口保存到配置。已有后端在运行时，目录部分照常交给它（立即生效），纯密码/端口修改则提示重启后生效。
