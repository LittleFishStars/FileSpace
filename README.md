...
- **🖥️ 配置文件驱动共享**：无参数运行自动读取（不存在则创建）用户配置目录下的默认配置文件 `filespace/config.yaml`，按其 `shared_folders` 共享；也可用 `-d/--dir` 临时指定共享文件夹；本机管理页添加/移除的共享会同步写回配置文件，命令行加 `--save` 也可把本次参数设置（目录/密码/端口/节点名称）追加保存到配置文件
...
│   │   └── state/        # 本地运行锁
│   ├── config.yaml       # 配置示例
│   └── version.go        # 版本号（当前 0.4.1-2609061802）
...
> **无家目录兜底**：若环境未定义 `$XDG_CONFIG_HOME` 与 `$HOME`（如精简容器、无人值守设备），不再报错退出，配置与运行锁文件直接存放在**可执行文件所在目录**（`config.yaml`、`lock`），单文件便携运行无需 `-c`。

文件不存在则自动创建带注释的模板（含 `shared_folders: []`，不共享任何文件夹）。可用 `-c, --config <文件>` 指定其它配置文件。仓库内 `backend/config.yaml` 为配置示例（监听端口、节点名称、共享目录列表、默认访问密码、mDNS 服务名等）。
...
**命令行 `--save`**：把本次命令行参数设置**追加**保存到当前配置文件后再继续运行，下次无参数启动即按新配置生效——`-d/--dir` 目录追加进 `shared_folders`（去重，保留原有条目与各文件夹显式密码）、`-P/--passwd` 覆盖顶层默认密码（`-P ''` 清除）、`-p/--port` 覆盖监听端口、`--hostname` 覆盖顶层节点名称（`--hostname ''` 清除，恢复系统主机名）。例：`filespace -d ~/docs -P secret --save` 共享 `~/docs`、设默认密码并一并保存；`filespace -p 9000 --save` 只把端口保存到配置。已有后端在运行时，目录部分照常交给它（立即生效），纯密码/端口/节点名称修改则提示重启后生效。

### 节点名称

每个节点在局域网主机列表里以**节点名称**显示，默认使用**系统主机名**。需要更友好的名字（如「客厅电脑」「书房 NAS」）时：
- **命令行临时指定**：`filespace --hostname 客厅电脑 -d ~/docs`（仅本次运行生效）；
- **持久化**：写入配置顶层 `hostname:`（如 `hostname: "客厅电脑"`），或 `filespace --hostname 客厅电脑 --save` 一键保存，下次无参数启动即生效；`--hostname ''`（或配置留空）恢复系统主机名。

自定义名称只影响**展示与上报**（其他节点看到的 `/api/node` 的 `hostname`）；**节点 ID 始终按系统主机名生成**，改名不会让其他节点把它当成新节点（mDNS 去重、自识别与退出通知均不受影响）。

### 运行锁

已有一个 `filespace` 后端在运行时（无论端口，通过配置目录下的运行锁文件 `lock` 标识——常规在用户配置目录 `filespace/` 下，无家目录环境在程序所在目录，文件内容为运行中后端的端口），再启动的后端进程检测到锁文件存在且内容端口存活，则仅允许用 `-d/--dir` 把目录追加给已运行的后端（通过 `POST /api/folders/add` 交给它，该接口仅允许本机回环地址调用），自身随即退出；无追加内容（如无参数）时提示并退出。后端正常退出会删除锁文件；进程崩溃残留的锁文件会在下次启动时被自动清理重建。
...
- **web 端**：本机管理页「添加共享文件夹」时可为所选文件夹设置独立访问密码；不想设置点弹窗左下「跳过」，直接以开放共享添加。
...
参数:
  ...
  --hostname <名称>       自定义本节点在局域网中显示的名称（默认用系统主机名；
                           ...
                           已有后端在运行时该参数不生效（配合 --save 保存后重启生效）
  -h, --help              显示本帮助信息
