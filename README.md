...
- **🌐 Web 界面**：浏览器查看局域网内其他节点及其共享文件夹，支持文件浏览、下载与在线预览（PDF / Office / 代码 / 图片 / 文本，二进制回退下载）；本机共享文件夹在独立的「本机管理」页中添加（点击按钮弹出系统原生目录选择器，不可用时回退手动输入）/ 删除，本机文件用系统默认应用直接打开
...
│   ├── config.yaml       # 配置示例
│   └── version.go        # 版本号（当前 0.3.2-2609021054）
...
| `GET /api/folders` | 本节点共享的文件夹列表（每个文件夹含 `auth` 标记：是否设置了访问密码） |
