...
同一端口已有后端在运行时（例如默认 8080 已被一个 `filespace` 实例占用），再启动的进程会先探测该端口：若探测到 filespace 后端，则仅允许用**目录参数或 `-a`** 把目录追加给已运行的后端（通过 `POST /api/folders/add` 交给它，该接口仅允许本机回环地址调用），自身随即退出，避免端口冲突；无追加内容（如无参数或 `--this`）时提示并退出。追加的目录同样会被记录，下次启动时一并恢复；重复追加同一目录（含符号链接指向同一目录）会被自动去重，但允许同时共享父目录与其子目录。
