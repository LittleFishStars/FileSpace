...
//   - -p/--port：覆盖监听端口；
//   - --hostname（显式给出，含空值清除）：覆盖配置顶层的节点名称
//     （后三者已由 loadConfig 应用到 cfg，此处直接随文件写回）。
//
...
func saveSettingsToConfig(cfg *config.Config, configPath string, opts *options) {
	...
	if opts.hasHost {
		if opts.hostname == "" {
			saved = append(saved, "节点名称已清除（恢复系统主机名）")
		} else {
			saved = append(saved, fmt.Sprintf("节点名称设为 %s", cfg.Hostname))
		}
	}
